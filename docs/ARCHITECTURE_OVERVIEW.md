...
- **Analytics account** – Hosts the data lake and analytics services. Data is replicated from the ingestion account into raw, cleaned and curated buckets. Glue/Lake Formation or ETL frameworks transform data from raw to curated zones. Analytical tools (Amazon EMR, Amazon Athena or Databricks) access curated data through fine‑grained IAM roles and Lake Formation permissions.  
- **Shared services account** – Contains centralized CI/CD pipelines, logging, security tools (GuardDuty, Config), and cross‑account IAM roles. This account centralizes CloudTrail and CloudWatch logs from all accounts, enabling unified security monitoring and compliance.  

The following mermaid diagram summarizes the high‑level data flows and account boundaries:  

```mermaid
graph TD
  subgraph IngestionAccount
    ProdSources[Producers_Apps_Devices] --> Stream[Kinesis_or_MSK]
    Stream --> S3RawIngest[S3_Raw_Zone]
  end
  subgraph AnalyticsAccount
    S3RawIngest --> S3Raw[Raw_Bucket]
    S3Raw --> ETL[Glue_or_ETL_Jobs]
    ETL --> S3Clean[Cleaned_Bucket]
    S3Clean --> S3Curated[Curated_Bucket]
    S3Curated --> AnalyticsPlatform[Analytics_Platform]
  end
  subgraph SharedServicesAccount
    CICD[CI/CD_and_Security_Tools] --> AllAccounts
    Logging[Central_Logging] --> AllAccounts
  end
```
